...
const normalizeGraphqlRepo = (repo: GraphqlRepo): GithubRepo => ({
  id: repo.id,
  name: repo.name,
...
const normalizeCommunityPinnedRepo = (repo: CommunityPinnedRepo): GithubRepo => ({
...
async function fetchPinnedViaGraphql(username: string, token: string) {
  const query = `
    query {
      user(login: "${username}") {
  ...
  return nodes.map(normalizeGraphqlRepo);
}

async function fetchPinnedViaCommunityApi(username: string) {
...
export async function GET(request: NextRequest) {
  ...
  if (token) {
    try {
      ...
      console.error("[api/github/pinned] GraphQL fetch failed", error);
    }
  }

  try {
    const fallbackPinned = await fetchPinnedViaCommunityApi(username);
    if (fallbackPinned.length > 0) {
      return NextResponse.json(fallbackPinned, { status: 200 });
    }

    return NextResponse.json([], { status: 200 });
  } catch (error) {
    console.error("[api/github/pinned] Community API fetch failed", error);
    return NextResponse.json({ error: "Unable to fetch pinned repositories" }, { status: 500 });
  }
}
